...
if __name__ == "__main__":
    ...
    # construct the MLP
    mlp_lr_linear_decay = MLP(784, [32, 16], 10,  init='rand')
    mlp_lr_nonlinear_decay = MLP(784, [32, 16], 10,  init='rand')
    ...
    # train the MLP
    mlp_lr_linear_decay.train(train_images, train_labels, val_images, val_labels, epochs=100, 
                        reg_method=reg_method, lambd=lambd,
                        learning_rate=0.0003, lr_decay=[True, 'linear', 5, 0.05],
                        batch_size=1, image_save_path="./linear_decay_learning_curve.png")
    mlp_lr_nonlinear_decay.train(train_images, train_labels, val_images, val_labels, epochs=100, 
                        reg_method=reg_method, lambd=lambd,
                        learning_rate=0.0003, lr_decay=[True, 'non-linear', 5, 0.95],
                        batch_size=1, image_save_path="./nonlinear_decay_learning_curve.png")

    # predictions on validation set
    mlp_lr_linear_decay.evaluate_model(val_images, val_labels, reg_method, lambd)
    mlp_lr_nonlinear_decay.evaluate_model(val_images, val_labels, reg_method, lambd)
