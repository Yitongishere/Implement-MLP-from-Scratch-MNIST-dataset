...
if __name__ == "__main__":
    ...
    # construct the MLP
    mlp_1More_Layer = MLP(784, [32, 8, 16], 10,  init='rand')
    mlp_1Less_Layer = MLP(784, [16], 10,  init='rand')
    ...
    # train the MLP
    mlp_1More_Layer.train(train_images, train_labels, val_images, val_labels, epochs=100, 
                        reg_method=reg_method, lambd=lambd,
                        learning_rate=0.0003, lr_decay=[False, 'non-linear', 5, 0.95],
                        batch_size=1, image_save_path="./1More_learning_curve.png")
    mlp_1Less_Layer.train(train_images, train_labels, val_images, val_labels, epochs=100, 
                        reg_method=reg_method, lambd=lambd,
                        learning_rate=0.0003, lr_decay=[False, 'non-linear', 5, 0.95],
                        batch_size=1, image_save_path="./1Less_learning_curve.png")

    # predictions on validation set
    mlp_1More_Layer.evaluate_model(val_images, val_labels, reg_method, lambd)
    mlp_1Less_Layer.evaluate_model(val_images, val_labels, reg_method, lambd)
